...
# - Use underscores because camel case is ugly. 
__all__ = ['DFStruct']


class DFStruct(object):
    """
    ...
    def __init__(self, **keyArgs):
        ...
        for name, val in keyArgs.items():
            self.__safe_add(name, val)

    def __safe_add(self, name, val):
        """
    ...
    def merge_items(self, struct, *name_list):
        """
        ...
        *name_list : `str`
        ...
        ------
        RuntimeError
            Raised if any item in name_list already exists in self 
            (but any items before the conflicting item
            in name_list will have been copied).
        ...
        for name in name_list:
            self.__safe_add(name, getattr(struct, name))
    ...
    def copy(self):
        ...
        -------
        copy : `DFStruct`
            One-level-deep copy of this DFStruct.
        """
        return DFStruct(**self.get_dict())
    ...
    def __repr__(self):
        itemList = ["%s=%r" % (name, val) for name, val in self.get_dict().items()]
